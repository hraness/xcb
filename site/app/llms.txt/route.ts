...
Managed ALGAL controllers ship in v0.6.0. Use \`xcb backlog program\` for an immediate controller or \`xcb schedules program\` for recurring work, with an explicit \`--managed-calls\` limit from 1 to 8. A current project grant authorizes every child. Controllers suspend for ordinary routed workers and resume only from their conclusively completed reports; waiting parents release their worker slot. \`xcb backlog program-status <task-id>\` and \`/program\` show the linked child, progress and receipts. Child questions and approvals still require the normal attention flow. See the [project-agent reference](https://github.com/hraness/xcb/blob/v0.6.0/docs/project-agents.md).
...
- [Learned routing & continuation](https://xcb.sh/docs/reflexes): reflexes that learn the model tier you want and when a worker stopped short, with forward-trial promotion and rollback. Auto-certification ships in v0.6.0: settle and confirm default to auto and observe until local evidence certifies them. Earlier v0.5.0 downloads default both to observe, require explicit active settings for them to act, and do not accept auto; see the [v0.5.0 reflex reference](https://github.com/hraness/xcb/blob/v0.5.0/docs/reflexes.md).
