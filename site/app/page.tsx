...
const questions = [
  ...
  { question: "Is the managed harness ready?", answer: "No, it is experimental. It is being rebuilt on ALGAL to propose routing rules and test them on labeled examples, and the current build does not execute self-modifying orchestration policies. What it learns today are reflexes: two small programs that pick a model tier for a new task and categorize how a turn ended. They learn from your replies, adopt new parameters only when held-out examples show an improvement, and can be rolled back. Provider checks, account locking, and run records stay fixed." },
