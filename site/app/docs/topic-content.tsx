...
xcb models`}</Code>
      ...
      <p>Plain <code>xcb</code> reopens the latest conversation for the workspace, creating one when none exists. Closing it detaches while tasks keep running. Use <code>/tasks</code> to inspect work and <code>/sessions</code> to switch control conversations. Ordinary prompts create new work. In v0.7, select a task in <code>/agents</code> and press <code>s</code> to guide it or <code>a</code> to answer its current question. The prompt displays its target; <code>/task</code> returns to new work.</p>
...
xcb plugins`}</Code>
      ...
      <p>In managed chat, <code>/sessions</code> switches control conversations, <code>/tasks</code> inspects the shared task swarm, and <code>/new</code> starts another conversation. In v0.7, <code>/task</code> writes new work; select a task in <code>/agents</code> and press <code>s</code> to guide it or <code>a</code> to answer its current question. To reopen a saved direct provider session, use <code>xcb resume</code>. In that direct terminal, <code>/sessions</code> selects provider sessions, <code>/new</code> starts another, and <code>/accounts</code> and <code>/model</code> open account and model pickers. Finish the current turn before changing its account or model.</p>
      <p>The v0.7 editor uses familiar Codex CLI keys: <code>Ctrl-R</code> searches prompt history, <code>Ctrl-T</code> opens saved messages, <code>F3</code> searches the transcript, and <code>Ctrl-G</code> opens your editor. <code>/drafts</code> recovers private input without sending it. In v0.8.7, <code>/vim</code> switches the composer to Vim-style modal editing: Esc enters Normal mode and Enter still sends. See the <a href="https://github.com/hraness/xcb/blob/main/docs/terminal.md" target="_blank" rel="noopener noreferrer">terminal guide ↗</a> for shortcuts, agent controls, and recovery limits.</p>
