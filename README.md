...
`XCB_INSTALL_PREFIX` changes the prefix. The TypeScript compatibility CLI
installs as `xcb-compat`, so it does not shadow the native `xcb`; an older
compatibility install that still used the `xcb` name should be removed, and
`command -v xcb` shows which binary answers. The installer also records a private install manifest under the prefix and
